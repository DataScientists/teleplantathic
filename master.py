...
def getRightTime():
    ...
    #datetime.datetime.now(timezone(ourTimezone) causes error if ourTimezone is "local" or ""
    
def logging(url, data): 
    #join the description into 1 comma separated string
    #print 'logging '+ str(data)
    #print 'logging data description ' + str(data['description']) + '\n'
    ...
    try:
        ...
        try:
            with open('log.txt', 'a') as outfile:
                data['description'] = data['description'] + ',log Connection Error'
...
def pumpWatering():
    ...
    try:
        ...
        if 'timezone' in data.keys():
            ...
            pumpStates["description"].append(ourTimezone)
            
        else:
            print "timezone not found"
            # pumpStates["type"] = "WARNING"
            pumpStates["description"].append("No timezone given so use local")
                ...
                if len(data[i]) > 0: 
                    ...
                    for j in data[i]:                     
                        ...
                        if((start_hour*3600+start_minute*60)  <= (now.hour*3600 + now.minute*60 + now.second) <=  (start_hour*3600+start_minute*60 + run_time*60)): 
                            ...
                            pumpStates["description"].append('start time ' + str(now))
                            
                            onTimeString = "turning on for " + str(run_time) + " minutes"
    ...
    #failed to get schedule. Load local one? 
        ...
        pumpStates["description"].append("Schedule Retrieve Connection Error")
        print pumpStates
        pumpRunning = False
        logging(postUrl, pumpStates) 
